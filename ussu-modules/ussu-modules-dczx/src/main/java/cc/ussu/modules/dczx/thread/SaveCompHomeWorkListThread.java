...
import cc.ussu.modules.dczx.model.vo.composite.ExamResultInfo;
...
public class SaveCompHomeWorkListThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            initExamListmcapiVo = JSONUtil.toBean(rep, InitExamListmcapiVo.class);
    ...
    private void analyzeResponse(InitExamListmcapiVo jsonObject, Map<String, List<String>> decodeParams) {
        ...
        for (ExamVo examVo : examList) {
            ...
            if (CollUtil.isEmpty(examResultInfos)) {
                return;
            }
            // 作业记录
            for (ExamResultInfo examResultInfo : examResultInfos) {
                ...
                if (newDcInterfaceLog != null) {
                    ...
                    SaveDczxCompPaperQuestionThread saveDczxCompPaperQuestionThread = new SaveDczxCompPaperQuestionThread(newDcInterfaceLog);
                    SpringUtil.getBean(ThreadPoolTaskExecutor.class).submit(saveDczxCompPaperQuestionThread);
                }
            }
        }
    }
    ...
    public boolean existsThis(String url) {
        IDcInterfaceLogService service = SpringUtil.getBean(IDcInterfaceLogService.class);
